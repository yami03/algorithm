...
function findEvenIndex(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    let next = i === 1 ? 1 : i;
    let left = i === 0 ? 0 : arr.slice(0, next).reduce((prev, current) => prev + current);
    let right = i === arr.length -1 ? 0 : arr.slice(i+1).reduce((prev, current) => prev + current);
...
assertEquals(findEvenIndex([1, 2, 3, 4, 3, 2, 1]), 3);
assertEquals(findEvenIndex([1, 100, 50, -51, 1, 1]), 1);
assertEquals(findEvenIndex([1, 2, 3, 4, 5, 6]), -1);
assertEquals(findEvenIndex([20, 10, 30, 10, 10, 15, 35]), 3);
