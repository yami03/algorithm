// Codewars Link: https://www.codewars.com/kata/58708934a44cfccca60000c4
...
// This Kata Series is based on a fictional story about a computer scientist 
// and engineer who owns a firm that sells a toy robot called MyRobot which 
// can interpret its own (esoteric) programming language called RoboScript. 
...
// You are a computer scientist and engineer who has recently founded a firm 
// which sells a toy product called MyRobot which can move by receiving a set of instructions by reading a file containing a script. 
// Initially you have planned the robot to be able to interpret JavaScript files for its movement instructions 
// but you later decided that it would make MyRobot too hard to operate 
// for most customers out there who aren't even computer programmers in the first place. 
...
// can easily learn how to write scripts in this language which would enable them to properly operate MyRobot. 
// However, you are currently at the initial stage of inventing this new Esolang. 
// The first step to popularize this (esoteric) scripting language is naturally to invent a new editor for it 
...
// And for multiple characters with the same color, 
// simply wrap them with a single < span > tag of the correct color:
...
function highlight(code) {
  ...
  for (let i = 0; i < code.length; i++) {
    if (i !== 0 && code[i] !== code[i - 1] && !Number.isInteger(Number(code[i - 1])) && !Number.isInteger(Number(code[i]))) {
      codeArray.push(code.slice(firstNumber, i));
      firstNumber = i;
    } else if (!Number.isInteger(Number(code[i - 1])) && Number.isInteger(Number(code[i])) || Number.isInteger(Number(code[i - 1])) && !Number.isInteger(Number(code[i]))) {
      codeArray.push(code.slice(firstNumber, i));
      firstNumber = i;
    }
    if (i === code.length - 1) { //마지막일 때

      if (firstNumber <= code.length - 2) { //이전에 것도 넣어줘야 할때 
        codeArray.push(code.slice(firstNumber));
      } else {
  ...
  codeArray = codeArray.filter(Boolean);
  let result = '';

  function tag(first, total) {
    if (first === '(' || first === ')') return total;
    if (Number.isInteger(Number(first))) return `<span style="color: orange">${total}</span>`;
    const tagOb = {
      F: `<span style="color: pink">${total}</span>`,
      L: `<span style="color: red">${total}</span>`,
      R: `<span style="color: green">${total}</span>`
    };
    return tagOb[first];
  }

  for (let le of codeArray) {
    result += tag(le[0], le);
  }
...
assertEquals(highlight("F3RF5LF7"), '<span style="color: pink">F</span><span style="color: orange">3</span><span style="color: green">R</span><span style="color: pink">F</span><span style="color: orange">5</span><span style="color: red">L</span><span style="color: pink">F</span><span style="color: orange">7</span>');
assertEquals(highlight("FFFR345F2LL"), '<span style="color: pink">FFF</span><span style="color: green">R</span><span style="color: orange">345</span><span style="color: pink">F</span><span style="color: orange">2</span><span style="color: red">LL</span>');
